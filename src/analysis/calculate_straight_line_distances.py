...
class StraightLineDistanceCalculator:
    ...
    def __init__(self, sawmills, harv_sites, out_csv, subset_size, workspace):
        ...
        desc = arcpy.Describe(self.harvest_sites)
        self.id = desc.OIDFieldName
        if desc.shapeType == "Polygon":
            if arcpy.Exists(f"{self.harvest_sites}_points"):
                arcpy.management.Delete(f"{self.harvest_sites}_points")
            arcpy.management.FeatureToPoint(self.harvest_sites, f"{self.harvest_sites}_points", "INSIDE")
            self.harvest_sites = f"{self.harvest_sites}_points"
    ...
    def create_output_dir(self):
        ...
        if self.out_dir != "#":
            ...
            self.out_dir = self.workspace + "/../" + "outputs/straight_line_distances"
            if not os.path.exists(self.out_dir):
                os.makedirs(self.out_dir)
    ...
    def process(self):
        ...
        arcpy.AddMessage(f"Straight Line Distance CSV can be found at: {os.path.abspath(self.out_csv)}")
