...
#          sawmills. Uses every site in the harvest site input. Outputs mean and median multipliers as well as circuity
    ...
    for sm_type in dist_id_dict:
        ...
        for i, oid in enumerate(oid_list):
            try:
                ...
                #time.sleep(0.5)
                gc.collect()
                arcpy.management.MakeFeatureLayer(harvest_sites, f"harvest_site_{oid}")
                arcpy.management.MakeFeatureLayer(sawmills, f"sawmill_layer_{oid}")
                ...
                arcpy.management.SelectLayerByAttribute(
                    ...
                    "NEW_SELECTION",
                    f"OBJECTID = {dist_id_dict[sm_type][oid][0]}"
                )
                ...
                if i < len(oid_list) - 1:
                    arcpy.AddMessage(
                        f"Attempting new ID: {oid_list[i + 1]}, {dist_id_dict[sm_type][oid_list[i + 1]][0]}"
                    )
                    continue
                else:
